    ...
    async handle(handlerInput) {
        ...
        if (key === "orderMore") {
            ...
            if (answer === "yes") {
                ...
                let string_contentList = stringifyOrderContents(orderContents);
...
const ProductConfirmation_EditOrderIntentHandler = {
  ...
  handle(handlerInput) {
    ...
    if (intent.slots.spokenProductName.confirmationStatus === "CONFIRMED") {
        ...
        return handlerInput.responseBuilder
          .speak("Please state a new quantity for " + sessionAttributes.resolvedProduct.resolvedProductName + " or say zero to remove?")
          .reprompt("Please state a number for the new quantity for " + sessionAttributes.resolvedProduct.resolvedProductName + " or say zero to remove?")
...
const ProductQuantityGiven_EditOrderIntentHandler = {
  ...
  async handle(handlerInput) {
    ...
    if(intent.slots.NewQuantity.value != 0){
        ...
        return handlerInput.responseBuilder
          .speak("Okay, I have changed " + sessionAttributes.resolvedProduct.resolvedProductName + " to be " +  intent.slots.NewQuantity.value + ". What else can I help you with?")
        ...
        return handlerInput.responseBuilder
          .speak("Okay, I removed " + sessionAttributes.resolvedProduct.resolvedProductName + " from your order. What else can I help you with?")
          .reprompt("What else can I help you with?")
          .getResponse();
    }
  }
...
const ViewNextDeliveryContentsIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        if(allItems === null){
           ...
           speakOutput = 'we did not find any orders scheduled for your next delivery date';
        }else{

           speakOutput = 'In your next order you have, ' + stringifyOrderContents(allItems) + ".";
        }
...
const ViewPendingOrderContentsIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        if(pendingOrder === null){
            speakOutput = 'you do not have a pending order please say start order to begin one';
            ...
            if(allItems !== null){
                console.log("guessing 1");
                  speakOutput = 'In your shopping cart you have, ' + stringifyOrderContents(allItems) + ".";
                  console.log("guessing 2");
            }else{
                  speakOutput = 'we did not find any items in your pending order/cart';
            }
...
const stringifyOrderContents = (orderContents) => {
    let stringList = "";
    console.log(orderContents);
    for (var i = 0; i < orderContents.length; i++) {
        console.log("happened once");
        const quantity = orderContents[i].quantity;
        const productName = orderContents[i].productName;
        if (i === 0) {
            stringList += "";
        } 
        else if (i === orderContents.length - 1) {
            if (orderContents.length === 2) {
                stringList += " and ";
            } else {
                stringList += ", and ";
            }
