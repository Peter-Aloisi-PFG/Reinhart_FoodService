...
async function getUserAccess(accesstoken) {
    return await Util.getUserAccess(accesstoken);
}
...
async function getUserInfo(email) {
    ...
    for(let key in customerData){
        if(customerData[key].Email !== undefined && customerData[key].Email === email){
            const customerNumber = customerData[key].CustomerNumber;
            const customerName = customerData[key].CustomerName;
...
async function getProductFromCatalogue(spokenProductName) {
    var foodData = await Util.getJSON('catalogue.json');
